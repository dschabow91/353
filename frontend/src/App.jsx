      ...
      <header className={tw("sticky top-0 z-20 border-b", dark ? "border-zinc-800 bg-gradient-to-r from-zinc-950 via-zinc-900 to-zinc-950/90 backdrop-blur":"border-slate-200 bg-gradient-to-r from-white via-sky-50 to-white/80 backdrop-blur")}>
        <div className="max-w-7xl mx-auto px-3 sm:px-6 py-3 flex items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            {user ? (<><button className={btn(dark,"ghost")} onClick={()=>window.dispatchEvent(new CustomEvent("nav:profile"))}>Profile</button><button className={btn(dark, "danger")} onClick={onLogout}>Logout</button></>) : null}
          </div>
        </div>
      </header>
...
function Home({ dark, user }){
  useEffect(()=>{ function go(){ setTab("Profile"); } window.addEventListener("nav:profile", go); return ()=> window.removeEventListener("nav:profile", go); },[]);
...
function WorkOrdersView({ dark, user }){
  ...
  return (
    <div className="space-y-2">
      ...
      {modal && <Modal dark={dark} onClose={()=>{setModal(false); setEditing(null);}}><WOForm dark={dark} initial={editing || { title:"", description:"", asset:"", location:"", priority:"Medium", assignedTo:"", status:"Open", dueDate:"", checklist:[] }} onSave={save} onCancel={()=>{setModal(false); setEditing(null);}} /></Modal>}
    </div>
  );
}

function WOForm({ dark, initial, onSave, onCancel }){
  ...
  return (
    <form className="space-y-3" onSubmit={e=>{e.preventDefault(); onSave(form);}}>
      ...
      <div className="flex items-center justify-end gap-2 pt-2"><button type="button" className={btn(dark,"ghost")} onClick={(e)=>{ e.preventDefault(); onCancel && onCancel(); }}>Cancel</button><button className={btn(dark)}>{initial?.id? "Save":"Create"}</button></div>
    </form>
  );
}
...
function PMView({ dark, user }){
  ...
  return (<div className="space-y-2">
    ...
    {modal && <Modal dark={dark} onClose={()=>{setModal(false); setEditing(null);}}><PMForm dark={dark} initial={editing || { asset:"", task:"", startDate: new Date().toISOString().slice(0,10), frequency:"Monthly", interval: 1 }} onSave={save} onCancel={()=>{setModal(false); setEditing(null);}} /></Modal>}
  </div>);
}
function PMForm({ dark, initial, onSave, onCancel }){ const [form, setForm] = useState(initial); function set(k,v){ setForm(p=>({...p,[k]:v})) } return (<form className="space-y-3" onSubmit={e=>{e.preventDefault(); onSave(form);}}><h3 className="text-lg font-semibold">{initial?.id? "Edit PM":"New PM"}</h3><div className="grid sm:grid-cols-2 gap-3">
...
</div><div className="flex items-center justify-end gap-2 pt-2"><button type="button" className={btn(dark,"ghost")} onClick={(e)=>{ e.preventDefault(); onCancel && onCancel(); }}>Cancel</button><button className={btn(dark)}>{initial?.id? "Save":"Create"}</button></div></form>);}
